...
package com.yxyhail.qrman.camera;
...
import com.yxyhail.qrman.camera.open.OpenCamera;
import com.yxyhail.qrman.camera.open.OpenCameraInterface;
...
//    setCameraDisplayOrientation(((Activity) context),requestedCameraId,cameraObject);
    }
    ...
    public synchronized Rect getFramingRect() {
        if (framingRect == null) {
            ...
            int wh, leftOffset, topOffset;
            if (screenResolution.x < screenResolution.y) { // 竖屏
                wh = screenResolution.x * 5 / 8;
                leftOffset = (screenResolution.x - wh) / 2;
                topOffset = (screenResolution.y - wh) / 2;
            } else {
                wh = screenResolution.y * 5 / 8;
                leftOffset = (screenResolution.x - wh) / 2;
                topOffset = (screenResolution.y - wh) / 2;
            }
            framingRect = new Rect(leftOffset, topOffset, leftOffset + wh, topOffset + wh);
        ...
        if (point.x < point.y) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++)
                    rotatedData[x * newW + newW - 1 - y] = data[x + y * width];
            }
            source = new PlanarYUVLuminanceSource(rotatedData, newW, newH,
